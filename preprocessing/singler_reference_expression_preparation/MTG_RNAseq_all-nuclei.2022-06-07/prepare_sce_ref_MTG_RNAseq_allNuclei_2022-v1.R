...
message("[1/8] Reading reference data...")
...
message("[2/8] Copying 'X' assay to 'counts'...")
assay(sce_ref_MTGRNAseqAllNuclei2022, "counts") <- assay(sce_ref_MTGRNAseqAllNuclei2022, "X")

message("[3/8] Removing 'X' assay...")
assays(sce_ref_MTGRNAseqAllNuclei2022) <- assays(sce_ref_MTGRNAseqAllNuclei2022)[
  setdiff(assayNames(sce_ref_MTGRNAseqAllNuclei2022), "X")
]

message("[4/8] Preparing gene IDs without version suffixes...")
orig <- sce_ref_MTGRNAseqAllNuclei2022
ids  <- sub("\\.\\d+$", "", rownames(orig))

message("[5/8] Aggregating counts across features...")
...
message("[6/8] Calculating log-normalized counts...")
sce_ref_clean <- scuttle::logNormCounts(sce_ref_clean)

message("[7/8] Converting assays to sparse matrices...")
...
message("[8/8] Saving cleaned reference object as RData...")

sce_ref_MTGRNAseqAllNuclei2022 <- sce_ref_clean

save(sce_ref_MTGRNAseqAllNuclei2022, file = "data/reference_celltype_expression_processed/sce_ref_MTGRNAseqAllNuclei2022.RData")
